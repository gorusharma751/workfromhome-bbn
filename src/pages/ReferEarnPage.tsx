...
import { mockUser, mockReferrals, referralSettings } from "@/data/mockData";
import { toast } from "sonner";

const ReferEarnPage = () => {
  const referralLink = `https://workfromhome.com/signup?ref=${mockUser.referralCode}`;
  ...
  const l1Refs = mockReferrals.filter((r) => r.level === 1);
  const l2Refs = mockReferrals.filter((r) => r.level === 2);
  const l3Refs = mockReferrals.filter((r) => r.level === 3);

  const totalEarnings = mockReferrals.reduce((s, r) => s + r.earnings, 0);
  const activeRefs = mockReferrals.filter((r) => r.isActive).length;
      ...
      <div className="mx-auto max-w-md px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <GlassCard className="mb-4 relative overflow-hidden">
            ...
            <p className="font-display text-2xl font-bold text-foreground mb-1">{mockUser.referralCode}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-4 grid grid-cols-3 gap-3">
          <GlassCard className="text-center py-4">
            ...
            <p className="font-display text-lg font-bold text-foreground">{mockReferrals.length}</p>
            <p className="text-[10px] text-muted-foreground">Total</p>
          </GlassCard>
          <GlassCard className="text-center py-4">
            <TrendingUp className="mx-auto h-5 w-5 text-success mb-1" />
            <p className="font-display text-lg font-bold text-foreground">{activeRefs}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <GlassCard className="mb-4">
            ...
            <div className="space-y-2">
              {[
                { level: 1, pct: referralSettings.l1Percentage, color: "primary" },
                { level: 2, pct: referralSettings.l2Percentage, color: "secondary" },
                { level: 3, pct: referralSettings.l3Percentage, color: "accent" },
              ].map((l) => (
                ...
                {refs.map((r) => (
                  <div key={r.id} className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2 mb-1">
                    <div className="flex items-center gap-2">
                      <div className={`h-2 w-2 rounded-full ${r.isActive ? "bg-success" : "bg-muted-foreground"}`} />
