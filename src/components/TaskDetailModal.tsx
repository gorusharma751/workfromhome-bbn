...
import { Copy, ExternalLink, Upload, CheckCircle2, X, MessageSquare } from "lucide-react";
...
const TaskDetailModal = ({ task, open, onClose }: TaskDetailModalProps) => {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(task.reviewText);
    setCopied(true);
          ...
          <div className="flex items-center justify-center gap-2">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center gap-2">
                ...
                {s < 3 && (
                  <div className={`h-0.5 w-8 rounded-full transition-all ${step > s ? "gradient-primary" : "bg-muted"}`} />
                )}
              </div>
            ))}
          </div>
            ...
            {step === 1 && (
              ...
              >
                <div className="rounded-xl bg-muted/50 p-4">
                  ...
                  <p className="text-sm leading-relaxed text-foreground">{task.reviewText}</p>
                </div>
                <Button
                  onClick={handleCopy}
                ...
                >
                  {copied ? (
                    ...
                    <>
                      <Copy className="h-4 w-4" /> Copy Review Text
                    </>
                  )}
                </Button>
              </motion.div>
              ...
              >
                <div className="rounded-xl bg-muted/50 p-4 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Open the task link, paste the copied review, and submit it.
                  </p>
                </div>
                <Button
                  onClick={handleOpenLink}
                  className="w-full bg-secondary border-0 font-display font-semibold text-secondary-foreground hover:bg-secondary/90"
                >
            ...
            {step === 3 && (
              ...
              >
                <div className="rounded-xl border-2 border-dashed border-border/50 p-8 text-center">
                  ...
                  <input type="file" accept="image/*" className="mt-2 w-full text-xs text-muted-foreground file:mr-2 file:rounded-lg file:border-0 file:gradient-primary file:px-3 file:py-1.5 file:text-xs file:font-medium file:text-primary-foreground" />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className="bg-muted/50 border-border/30 resize-none"
                    rows={3}
                  />
                </div>
                <Button
                  onClick={handleSubmit}
                  className="w-full bg-success border-0 font-display font-semibold text-success-foreground hover:bg-success/90"
                >
